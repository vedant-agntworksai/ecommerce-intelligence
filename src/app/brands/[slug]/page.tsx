...
export default async function BrandPage({params}:{params:Promise<{slug:string}>}){
  ...
  const s=data.summary;const retailerMap=new Map(data.retailerSummary.map((r:any)=>[r.retailer,r]));
  return <div className="p-8 space-y-8">
    ...
    {data.competitorBrands.length>0&&<BarList title="Most frequently matched competitor brands" points={data.competitorBrands.map((x:any)=>({label:x.brand,value:x.matches}))}/>}
