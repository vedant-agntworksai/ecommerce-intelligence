...
export function NewScrapeForm({defaultMaxReviews=500,defaultCompetitors=3}:{defaultMaxReviews?:number;defaultCompetitors?:1|3|5|10}){
 ...
 maximumReviewsPerProduct:Number(f.get("maximumReviews")||defaultMaxReviews),competitorsPerProduct:Number(f.get("competitors")||defaultCompetitors),forceRefresh:f.get("refresh")==="force"};
 ...
 <div className="grid md:grid-cols-2 gap-4"><label className="text-sm">Maximum reviews / product<input name="maximumReviews" type="number" defaultValue={defaultMaxReviews} min="1" max="5000" className="mt-2 w-full rounded-xl bg-slate-950 border border-slate-700 px-3 py-2"/></label><label className="text-sm">Competitors / product<select name="competitors" defaultValue={String(defaultCompetitors)} className="mt-2 w-full rounded-xl bg-slate-950 border border-slate-700 px-3 py-2">{[1,3,5,10].map(n=><option key={n}>{n}</option>)}</select></label></div>
