...
export function NewScrapeForm(){
 ...
 const body={brand:f.get("brand"),startUrl:f.get("startUrl"),retailer:f.get("retailer"),discoverProducts:true,collectPdp:true,matchCanonical:true,findCompetitors:true,
 ...
 const r=await fetch("/api/jobs",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify(body)});const data=await r.json();setBusy(false);setMessage(r.ok?`Job ${data.id} queued`:data?.message??"Unable to queue job");}
 return <form onSubmit={submit} className="card p-6 space-y-5"><div><h2 className="text-lg font-semibold">New scrape</h2><p className="muted text-sm mt-1">One workflow for products, reviews, matching, and competitors.</p></div>
 ...
 <div className="grid md:grid-cols-2 gap-3 text-sm"><label><input type="checkbox" name="reviews" className="mr-2"/>Collect product reviews</label><label><input type="checkbox" name="competitorReviews" className="mr-2"/>Collect competitor reviews (on demand)</label></div>
 <div className="grid md:grid-cols-2 gap-4"><label className="text-sm">Maximum reviews / product<input name="maximumReviews" type="number" defaultValue={500} className="mt-2 w-full rounded-xl bg-slate-950 border border-slate-700 px-3 py-2"/></label><label className="text-sm">Competitors / product<select name="competitors" defaultValue="3" className="mt-2 w-full rounded-xl bg-slate-950 border border-slate-700 px-3 py-2">{[1,3,5,10].map(n=><option key={n}>{n}</option>)}</select></label></div>
