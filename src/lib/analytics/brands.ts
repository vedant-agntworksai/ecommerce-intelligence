...
export async function getBrandDashboard(db:Database,slug:string){
  ...
  const summary=(await db.query<any>(`
    ...
    LEFT JOIN (
      SELECT canonical_product_id,COUNT(DISTINCT retailer) retailer_count
      FROM retailer_products GROUP BY canonical_product_id
    ) cov ON cov.canonical_product_id=cp.id
    WHERE cp.brand_id=$1
  `,[brand.id])).rows[0]??{};
  ...
  const competitorBrands=(await db.query<any>(`
    ...
    WHERE source.brand_id=$1
    GROUP BY cb.id,cb.name ORDER BY matches DESC LIMIT 12
  ...
  `,[brand.id])).rows.map(r=>({
    ...r,retailer_count:n(r.retailer_count),lowest_price:nullable(r.lowest_price),
    average_rating:nullable(r.average_rating),total_reviews:n(r.total_reviews),
  ...
  return {
    brand,
    summary:{
      uniqueProducts:n(summary.unique_products),retailerListings:n(summary.retailer_listings),
      productsWithReviews:n(summary.products_with_reviews),totalReviews:n(summary.total_reviews),
      averageRating:nullable(summary.average_rating),averagePrice:nullable(summary.average_price),
      competitorProducts:n(summary.competitor_products),crossRetailerMatches:n(summary.cross_retailer_matches),
      lastUpdated:summary.last_updated??null,
    },
    retailerSummary,competitorBrands,products,
  };
}

function parseImages(value:unknown):string[]{
  if(typeof value!=="string")return[];
  try{const v=JSON.parse(value);return Array.isArray(v)?v.filter(x=>typeof x==="string"):[];}catch{return[];}
}
