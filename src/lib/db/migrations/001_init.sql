...
CREATE TABLE IF NOT EXISTS canonical_products (
  ...
  normalized_quantity DOUBLE PRECISION, normalized_unit VARCHAR, pack_quantity INTEGER, description VARCHAR,
...
CREATE TABLE IF NOT EXISTS retailer_products (
  id VARCHAR PRIMARY KEY, canonical_product_id VARCHAR, retailer VARCHAR NOT NULL, retailer_product_id VARCHAR NOT NULL,
  retailer_sku VARCHAR, asin VARCHAR, product_url VARCHAR NOT NULL, canonical_url VARCHAR, title VARCHAR,
  price DOUBLE PRECISION, original_price DOUBLE PRECISION, currency VARCHAR, availability VARCHAR, seller VARCHAR, fulfilled_by VARCHAR,
  rating DOUBLE PRECISION, review_count BIGINT, upc VARCHAR, gtin VARCHAR, ean VARCHAR, model_number VARCHAR, mpn VARCHAR,
...
CREATE TABLE IF NOT EXISTS reviews (
  ...
  rating DOUBLE PRECISION, title VARCHAR, review_text VARCHAR, reviewer_name VARCHAR, verified_purchase BOOLEAN,
...
CREATE TABLE IF NOT EXISTS competitor_relationships (
  ...
  score DOUBLE PRECISION, rationale_json VARCHAR, updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
...
CREATE TABLE IF NOT EXISTS jobs (
  ...
  result_json VARCHAR, error VARCHAR, created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP, started_at TIMESTAMP, finished_at TIMESTAMP
);
CREATE TABLE IF NOT EXISTS historical_snapshots (
  id VARCHAR PRIMARY KEY, canonical_product_id VARCHAR, retailer_product_id VARCHAR,
  price DOUBLE PRECISION, rating DOUBLE PRECISION, review_count BIGINT, availability VARCHAR, captured_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP
);
CREATE TABLE IF NOT EXISTS resource_cache (
  resource_key VARCHAR NOT NULL, resource_kind VARCHAR NOT NULL, payload_json VARCHAR NOT NULL,
  updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP, PRIMARY KEY(resource_key, resource_kind)
);
