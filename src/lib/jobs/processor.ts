...
export async function processScrapeJob(payload:ScrapeJobPayload){
 ...
 for(const ref of limited){
   const id=ref.retailerProductId;
   const existing=id?await products.findRetailerProduct(payload.retailer,id):null;
   if(existing&&!payload.forceRefresh){reused++;continue;}
   if(!payload.collectPdp)continue;
   const product=await adapter.scrapeProduct(ref); await products.upsertRetailerProduct(product,null); scraped++;
 ...
 return{brandId:brand.id,discovered:refs.length,considered:limited.length,scraped,reused,newReviews,note:"Canonical association and competitor expansion are queued as separate phases once identifiers are persisted."};
}
