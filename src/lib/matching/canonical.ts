...
export function matchCanonical(incoming:NormalizedRetailerProduct,candidates:CanonicalProduct[]):CanonicalMatch{
  ...
  const incomingFunction=normalizeText(incoming.functionalCategory||incoming.productType||incoming.subcategory||incoming.category);
  const strong=candidates.find(c=>normalizeText(c.brand)===brand
    && !!incomingFunction && normalizeText(c.functionalCategory||c.productType||c.subcategory||c.category)===incomingFunction
    && (!incoming.formulation||!c.formulation||normalizeText(c.formulation)===normalizeText(incoming.formulation))
