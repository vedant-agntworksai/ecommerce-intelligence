...
export abstract class OxylabsRetailerAdapter implements RetailerAdapter {
  ...
  async scrapeProduct(ref:ProductReference):Promise<NormalizedRetailerProduct>{
    ...
    const {data}=await this.scraper.getOrFetch<any>(`${this.retailer}:pdp:${id}`,"pdp_static",{source:this.sourceFor("product"),url:ref.url,parse:true});
  ...
  async discoverCompetitorCandidates(product:CanonicalProduct,options:CompetitorSearchOptions):Promise<ProductReference[]>{
    ...
    if(!q)return[];
    const url=this.searchUrl(q);
    return (await this.discoverProducts(url,"")).slice(0,Math.max(options.limit*3,10));
  }
  async scrapeReviews(product:NormalizedRetailerProduct,options:ReviewOptions):Promise<NormalizedReview[]>{
    const {data}=await this.scraper.getOrFetch<any>(`${this.retailer}:reviews:${product.productId}`,"rating",{source:this.sourceFor("reviews"),url:product.productUrl,parse:true});
    return this.normalizeReviews(data,product.productId).slice(0,options.maxReviews);
  }
  ...
  protected normalizeProduct(data:any,url:string,id:string):NormalizedRetailerProduct{
    ...
    return {
      ...
      productType:p.product_type??null,targetUse:p.target_use??null,formulation:p.formulation??null,sizeText:p.size??null,
      normalizedQuantity:null,normalizedUnit:null,packQuantity:p.pack_quantity??null,productUrl:url,canonicalUrl:p.canonical_url??null,
      price:typeof p.price==="number"?p.price:null,originalPrice:typeof p.original_price==="number"?p.original_price:null,
      currency:p.currency??null,availability:p.availability??null,seller:p.seller??null,fulfilledBy:p.fulfilled_by??null,
      ...
      specifications:p.specifications&&typeof p.specifications==="object"?p.specifications:{},variants:Array.isArray(p.variants)?p.variants:[],
      scrapedAt:new Date().toISOString(),raw:data
    };
  }
  protected normalizeReviews(data:any,productId:string):NormalizedReview[]{
    const r=data?.results?.[0]?.content??data?.content??data;
    const rows=Array.isArray(r?.reviews)?r.reviews:Array.isArray(r)?r:[];
    return rows.filter((x:any)=>x?.id||x?.review_id).map((x:any)=>({
      ...
      rating:typeof x.rating==="number"?x.rating:null,title:x.title??null,reviewText:x.text??x.review_text??null,
      reviewerName:x.author??x.reviewer_name??null,verifiedPurchase:x.verified_purchase??null,
      reviewDate:x.date??x.review_date??null,helpfulVotes:x.helpful_votes??null,variant:x.variant??null,scrapedAt:new Date().toISOString()
    }));
  }
}
